...
| 1-6e | 🔴 | [~] **Bisect B** — **B1 (HWP off only) done 2026-09-16: worst 640 µs, >50 µs ×94, >400 ×6 ≈ A → `intel_pstate=disable` is NOT the effective component. B2 (`idle=poll`, HWP on) pending — expected to be the one.** | **B1** `tune rt-nohwp` (RT tuned + `intel_pstate=disable`, idle path unchanged) → `soak 30`; **B2** `tune rt-idlepoll` (RT tuned + `idle=poll max_cstate=0`, HWP on) → `soak 30` | the one that reaches ~43 µs is the cause. B1 → production = `intel_pstate=disable` (housekeeping keeps per-CPU governor via acpi-cpufreq). B2 → idle-path cause. Neither → the combination is needed (use B as-is, cost = all-core polling) | 2 × (reboot + 30 min) |
| 1-1b | 🔴 | [ ] **Re-run D1 with PM locked (D): generic + isolation + the winning setting from 1-6e** | boot `a1-bsw-generic-tuned` → `iso_pm on` → `soak 30`; compare with C | the 1-1 comparison was done with PM *on* for both kernels, so generic's 935 µs may have been PM reaction, not the kernel. If D ≤ 100 µs worst → generic is viable (removes the unsupported NVIDIA build risk; 2-1 throughput question disappears); if D ≫ C → keep RT | 30 min + reboot |
